...
const output = isBackendDevMode ? `${tars.options.build.path}/${tars.config.fs.staticFolderName}/css/` : `./dev/${tars.config.fs.staticFolderName}/css/`;
...
module.exports = function generateTaskContent(browser) {
    ...
    return gulp.src(stylesFilesToCompile)
        ...
        .pipe(gulpif(isBackendDevMode, rename({ suffix: '.min' })))
